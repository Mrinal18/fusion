...
from fusion.model import ABaseModel, ModelOutput
...
class Dim(ABaseModel):
    ...
    def _find_dim_in(self, conv_latent_size: int, source_id: int) -> int:
        batch_size = 2
        dim_in = 1
        dim_conv = None
        ...
        x = dummy_batch
        for layer in self._encoder[str(source_id)].get_layers():
