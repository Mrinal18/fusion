...
class MNIMaskTransform(Transform):
    def __init__(
        self,
        template='MNI152_T1_3mm_brain_mask_dil_cubic192.nii.gz',
        p=1
    ):
        super(MNIMaskTransform, self).__init__(p=p)
        self._mask = ~torch.BoolTensor(
            nib.load(template).get_fdata()).unsqueeze(0)

    def apply_transform(self, volume):
        for key, vol in volume.items():
            if 'view' in key:
                volume[key]['data'] = self.process_volume(
                    vol['data'])
        return volume
    ...
    def process_volume(self, volume, start=None, end=None):
        if start is not None and end is not None:
            ...
            start = np.random.randint(
                0, high=volume.shape[1] - self.shape, size=3)
